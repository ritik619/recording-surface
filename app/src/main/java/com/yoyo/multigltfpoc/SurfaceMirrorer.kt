...
class SurfaceMirrorer(
    ...
    fun onFrame() {
        Log.i("onFrame",surfaceMirrors.size.toString())
        surfaceMirrors.forEach { mirror ->
            Log.i("onFraME","${surfaceMirrors[0].swapChain.toString()}")
            Log.i("onFraME","${surfaceMirrors[0].swapChain.toString()}  ${getLetterboxViewport(view.viewport, mirror.viewport).height}")
            try{
            if (mirror.swapChain != null) {
                ...
                Log.i("EROR","${e.message}")
            }
        }
        Log.i("onFrame","completed")
    ...
    fun startMirroring(
        ...
        Log.i("startMirror",surfaceMirrors.size.toString())
        surfaceMirrors.add(
            SurfaceMirror(
                destSurface,
    ...
    fun stopMirroring(destSurface: Surface) {
        Log.i("stop",surfaceMirrors.size.toString())
